...
- [ ] Task: Dockerize Web Server
  - [ ] Write a production-ready `Dockerfile` and `.dockerignore`.
  - [ ] Add `deploy` target to `justfile` using `gcloud run deploy`.
- [ ] Task: Basic / IAP Authentication
  - [ ] Add header check middleware in `src/server.py` to validate user identity.
